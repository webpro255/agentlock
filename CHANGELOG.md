...
Suite: **1591 passed, 26 skipped on CPython 3.14.6 with no optional extras**, **1608 passed, 9 skipped on CPython 3.14.6 with `mcp 2.2.0`, `fastapi`, `flask` and PyNaCl**, and **1609 passed, 8 skipped on CPython 3.13.14 with the same extras**. 0 failed in each. Under `mcp 1.30.0` the engine suite is **1588 passed, 25 skipped, 0 failed**, with the review file's four `mcp` cases deselected: they construct `Server(name, on_call_tool=...)`, which is the 2.x constructor, so they cannot run against a 1.x SDK at all. The engine's 1.x hook is covered instead by `tests/test_v110_hardening.py`, which runs in every environment. `mypy agentlock/ --ignore-missing-imports` reports 0 errors and `ruff check .` is clean.
