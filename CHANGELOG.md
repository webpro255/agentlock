...
The integration hardening release. An external review of the published 1.9.1 wheel found seven groups of issue and supplied a runnable 33-test oracle for them, which is committed verbatim as `tests/test_v110_system_review.py` and is the definition of done for this release. Six of the seven are places where the engine's own execution paths did not carry a decision the gate had already made. This release adds no detection feature and no new schema field. It adds exactly one denial reason, `role_mismatch`, which came out of the first red pass and not out of the seven groups; the two reasons it newly attaches at commit time, `param_lineage` and `novel_lineage`, are the ones `authorize()` already used. Two pre-release red passes were then run against the built wheel, before any of it was published, and they found six more things; those close inside 1.10.0 rather than in a patch release, because 1.10.0 had not shipped. Credit for the findings in this release goes to an external review of the 1.9.1 wheel and to pre-release red passes against the built wheel.

**This release is not additive.** Several of the fixes change behavior on paths that were failing open, and code that relied on those paths will be denied or transformed where it previously was not. That is the point of them. Two shapes of caller are affected directly and should read the Security section before upgrading. **A caller that supplied a role for a principal who also holds an authenticated session** was believed, and is now denied with `role_mismatch` unless the claim and the session agree: a host that used the request role to raise or lower a session's privileges has to stop doing that and re-authenticate the principal instead. **A caller holding a token that was never bound to the call policy actually produced** now finds the grant bound to the effective parameters rather than to the requested ones. A two-step caller that hands `execute()` the parameters it authorized with is still recognized, through the second hash the token now carries, and is executed against the grant; a caller that reconstructed the parameter hash itself, or that relied on a grant outliving the transformation it was issued under, is not. The Security section says exactly which calls change.

Suite: **1616 passed, 9 skipped on CPython 3.14.6 with `mcp 2.2.0`, `fastapi 0.141.1`, `flask 3.1.3` and PyNaCl 1.6.2**, **1617 passed, 8 skipped on CPython 3.13.14 with those same four**, **1595 passed, 30 skipped on CPython 3.14.6 with `fastapi 0.135.3`, `flask 3.1.3` and PyNaCl 1.6.2 but no `mcp`**, and **1595 passed, 26 skipped on CPython 3.13.14 with `mcp 1.30.0` and neither web framework**. 0 failed in each. The last of the four also deselects the review file's four `mcp` cases: they construct `Server(name, on_call_tool=...)`, which is the 2.x constructor, so they cannot run against a 1.x SDK at all. The engine's 1.x hook is covered instead by `tests/test_v110_hardening.py`, which runs in every environment. Of the 30 skips in the third environment, 22 are guarded on `mcp` and 1 on `autogen`, and the remaining 7 are the pre-increment-3 baselines that have stood down since 1.7.0; none is guarded on a web framework, because both are installed there. `mypy agentlock/ --ignore-missing-imports` reports 0 errors and `ruff check .` is clean.
...
- **The authenticated session's role is authoritative over the caller's claim.** `authorize()` resolved the session's role only when the caller supplied none: `if session and not role: role = session.role`. A caller that supplied one was believed. The gate therefore held two facts about the same principal, one established out of band and one asserted in the request, and preferred the asserted one. With alice authenticated at role `user`, `authorize("admin_task", user_id="alice", role="admin")` was ALLOW for a tool restricted to `admin`, and through the MCP wrapper with no default configured a client that could name alice ran that tool over her session. When a session exists for `user_id` and a nonempty claimed role differs from it, the call is now denied with the new `DenialReason.ROLE_MISMATCH`, wire value `role_mismatch`, before the tool is looked up and before any policy step runs. Omitting the role still resolves it from the session, and with no session the caller's role is still trusted, which trusts the HOST to have authenticated the caller and is now documented as doing so. Credited to a pre-release red pass against the built wheel.
- **`role_mismatch` is a new denial reason and not a reuse of `insufficient_role`.** A role the tool does not allow and a role the session contradicts are different findings, and an auditor reading the log should not have to infer which one happened. The denial carries `required_role` as the session's role and `current_role` as the claim.
- **A declared output transformation now covers every shape a tool returns, not only `str`.** 1.10.0 threaded the output modifier onto all six execution paths, and each of them then applied it behind `isinstance(result, str)`. A tool returning `{"note": ssn}`, `[ssn]`, a nested structure, a tuple or bytes came back with the SSN intact although the modifier had reached the call. Returning a mapping is the ordinary case for a tool, so the declared transformation was still inert for most tools that declared one. The new `agentlock.modify.apply_output_modifier` walks the return value: `str` is modified; `dict`, `list` and `tuple` are walked recursively with every `str` leaf modified and the container type preserved, a named tuple rebuilt through `_make`; `bytes` are decoded as UTF-8 with `errors="replace"`, modified and re-encoded; anything else is returned unchanged. Dictionary keys are not modified, because a key is a field name and renaming fields would corrupt the payload the transformation was asked to sanitize. Applied at `gate.execute` (and so at `gate.call`, the sync decorator and the AutoGen map), in the async decorator, and in both MCP hooks, where a result that is not a content-carrying model now goes through the same walk. Credited to a pre-release red pass against the built wheel.
- **HTTP tool selection was also re-examined and needed no change.** The same red pass reported that with `tool_name_from_path` configured, a route the callback declines by returning `None` fell back to consulting `X-AgentLock-Tool`, letting the client pick the tool for that route. Measured against the built wheel, in both frameworks, it does not: the mapping's `None` is taken as "no tool name" and the request takes the existing pass-through, with the header never read for tool selection while a mapping is configured. That is the intended rule and it was already in both docstrings. Flask had a test pinning the declined-route branch and FastAPI had none, which is how a correct branch comes to be reported as a defect; FastAPI now has one.
- **An MCP result's structured payload is transformed too, not only its content blocks.** A `CallToolResult` carries two payloads: `content`, the list of content blocks, and `structured_content`, the machine-readable answer a client reads it for. The applier rewrote the `text` of every content block and never looked at the second field, so a handler putting the same value in both returned one copy redacted and the other intact, in the same return value. Both SDK majors are affected and they spell the field differently, `structuredContent` at 1.x and `structured_content` at 2.x, so the applier now tries both names and walks whichever is present through `apply_output_modifier` before setting it back. A result that is a plain mapping or a bare list of content items, which the 1.x handler contract allows, was already walked and still is. Credited to a second pre-release red pass against the built wheel.
- **The output walk covers `set` and `frozenset`.** `apply_output_modifier` covered `str`, `bytes`, `dict`, `list` and `tuple` and named sets among the types it returned unchanged. A set of strings is an ordinary return for a tool that answers with distinct values, so naming the type as uncovered documented a leak rather than bounding one. Members are now walked and the type rebuilt, so a `set` comes back a `set` and a `frozenset` comes back a `frozenset`; a subclass of either is rebuilt as the plain type, as `dict` and `list` subclasses already were. Credited to the same red pass.
- **Three limits of the output walk are now stated in its docstring rather than left to be discovered.** Dictionary KEYS are not modified, because a key is a field name and renaming fields would corrupt the payload the transformation was asked to sanitize. Arbitrary objects are returned as they came, whatever their `__str__` says, because the walk will not guess at a type it was not told how to rebuild. Bytes that are not valid UTF-8 are decoded lossily before the transformation sees them, so the readable part is transformed and the unreadable part is neither transformed nor preserved. A host returning any of those three has to redact it itself. No behavior changed here; the same red pass reported the three and they are stated, not closed.

### Limits

Four things this release deliberately does not do. Each is stated here, and in the docstring of the code that owns it, so that a host reads the boundary rather than discovering it.
...
- **Arbitrary objects are returned as they came.** The walk covers `str`, `bytes`, `dict`, `list`, `tuple`, `set` and `frozenset`, and rebuilds each of those. Anything else is handed back untouched, whatever its `__str__` says, because the walk will not guess at a type it was not told how to rebuild and will not mutate a caller's object in place. Related: bytes that are not valid UTF-8 are decoded lossily before the transformation sees them, so the readable part is transformed and the unreadable part is neither transformed nor preserved.
- **The `__wrapped__` boundary is where the gate's guarantee ends.** The gate binds to the callable's signature as `inspect` reports it, following `__wrapped__`, and 1.10.0 rebuilds each call from the effective parameters through that same binding. A wrapper that advertises one signature and alters the arguments before calling the inner function is the application's own code, on the trusted side of the boundary, and is outside what the gate can check. What the gate guarantees is that the call it authorizes is the call it binds, and that the wrappers invoke the function from that same binding.
- **The HTTP adapters pass no body parameters, so the per-parameter checks do not run on an HTTP request.** `AgentLockMiddleware`, the FastAPI dependency and both Flask entry points call `gate.authorize(tool_name, user_id=..., role=...)` and nothing else. They authorize the identity, the role, the session, the rate limit and the tool's own permission block, which is what a request-scoped gate can see before a handler has parsed a body. They do not reach scope limits, parameter lineage, novel lineage, a declared parameter transformation or the output modifier, and neither adapter calls `execute()`. A deployment that needs the execution contract on an HTTP route has to apply it inside the handler, by decorating the tool the handler calls or by going through `gate.call()`. This is unchanged by 1.10.0 and is stated because 1.10.0 is the release that makes the execution contract worth asking about.
...
- **`agentlock.modify.apply_output_modifier` is new and public.** It is the single applier every execution path now uses, so the shape of a tool's return no longer decides whether a declared transformation runs. `ModifyEngine.build_output_modifier` is unchanged and still returns a `str -> str` callable; the walk is beside it, not inside it.
