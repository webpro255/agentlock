...
For 1.10.1 it is 1675 passing and 9 skipped on CPython 3.14.6 with `mcp
2.2.0`, `fastapi`, `flask` and PyNaCl present, the 59 added tests being the
32 cases the reviewer appended to the oracle in their 1.10.0 recheck, which
brings that file to 65, and the 27 engine tests covering what those 32 reach
from outside cannot: the same MCP shapes over the 1.x hook, the resolved path
a whitelisted callable actually receives, and the recipient edge forms.
Without `mcp`, and with `fastapi`, `flask` and PyNaCl still present but no
`autogen`, it is 1641 passing and 43 skipped; 17 of the 43 are new in this
release and all 17 are guarded on `mcp`, being 10 oracle cases and 7 engine
cases. Nothing fails.
